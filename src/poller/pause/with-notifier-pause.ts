import { catchError, distinctUntilChanged, merge, NEVER, Observable, race, share, switchMap, take } from 'rxjs';
 ...
 * If notifier$ never emits, polling starts (same as resume). If notifier$ errors, the error is caught and polling is neither errored nor completed; the previous pause state is kept.
 *
...
export function withNotifierPause$<T>(poller$: Observable<T>, notifier$: Observable<boolean>): Observable<T> {
   ...
   * - Subscribers of sharedNotifier$ are not direct subs to notifier$; if notifier$ is a BehaviorSubject, "last value" only applies to direct subs, so the second sub (race) gets nothing -> defaultNotifier$ wins -> spurious false
